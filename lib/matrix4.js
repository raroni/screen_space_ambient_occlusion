...
Matrix4.createInversePerspective = function(fieldOfView, aspectRatio, near, far) {
  var matrix = new Matrix4();

  var halfFovTangent = Math.tan(fieldOfView/2);
  var farMinusNear = far-near;
  var twoFarNear = 2*far*near;

/*
  matrix.components[0] = halfFovTangent;
  matrix.components[5] = halfFovTangent/aspectRatio;
  matrix.components[11] = (far-near) / (2*far*near);
  matrix.components[14] = 1;
  matrix.components[15] = (far+near) / (2*far*near);
*/

  matrix.components[0] = halfFovTangent;
  matrix.components[5] = halfFovTangent/aspectRatio;
  matrix.components[11] = (near-far) / (2*far*near);
  matrix.components[14] = 1;
  matrix.components[15] = (far+near) / (2*far*near);
